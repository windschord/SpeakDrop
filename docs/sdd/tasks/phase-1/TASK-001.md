...
### 2026-02-24
- **作業内容**: pyproject.toml・speakdrop/パッケージ・tests/ディレクトリの作成
- **発生した問題**: `tool.uv.dev-dependencies` が非推奨（警告）。設計書準拠のため現状維持
- **解決方法**: 設計書の仕様に合わせて `[tool.uv] dev-dependencies` 形式を採用（uv sync は成功）
- **コミットハッシュ**: 78c32c7
